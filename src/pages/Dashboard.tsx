...
import { FileDown, Zap, Loader2, MessageSquare, BrainCircuit } from "lucide-react";
          ...
          <div className="flex items-center gap-4">
            {currentPage === "dashboard" && (
              <>
                ...
                <Button onClick={handleExportPDF} className="bg-gradient-to-r from-[#f90f54] to-[#8735d2] text-white font-bold px-6 shadow-lg transition-all hover:scale-105">
              ...
              <div ref={dashboardRef} className="space-y-8 animate-in fade-in duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                  {[
                    ...
                    <div key={i} className="bg-[#1e293b]/40 backdrop-blur-md border border-slate-700/50 p-6 rounded-[20px] shadow-xl">
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{kpi.label}</span>
                      <p className={`text-3xl font-bold mt-2 tracking-tight ${kpi.color}`}>{kpi.val}</p>
                      <p className="text-[10px] text-slate-500 mt-3 font-medium uppercase opacity-60">{kpi.sub}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 space-y-8">
                    {/* Tabela de Projetos */}
                    <div className="bg-[#1e293b]/40 backdrop-blur-md border border-slate-700/50 rounded-[24px] p-7 shadow-2xl">
                    ...
                    {/* Funil Waterfall */}
                    <div className="bg-[#1e293b]/40 backdrop-blur-md border border-slate-700/50 rounded-[24px] p-7 shadow-2xl">
                      <h3 className="text-sm font-bold mb-10 uppercase text-slate-400 tracking-[0.15em]">Funil de Conversão Integrado</h3>
                      <div className="space-y-10">
                        {funnelData?.map((f: any, i: number) => (
                          <div key={i} className="group">
                            ...
                            <div className="w-full bg-slate-800/50 h-7 rounded-lg overflow-hidden border border-slate-700/30">
                  ...
                  <div className="space-y-8">
                    {/* CIA Insights */}
                    <div className="bg-[#1e293b]/40 backdrop-blur-md border-t-[3px] border-t-[#f90f54] border-x border-slate-700/50 border-b border-slate-700/50 p-7 rounded-[24px] shadow-2xl">
                      <div className="flex items-center gap-3 mb-6 text-[#f90f54]">
                        <Zap className="w-5 h-5 fill-current" />
                        <h3 className="font-black text-xs uppercase tracking-[0.2em]">CIA Strategic Insights</h3>
                      </div>
                      <div className="space-y-5 text-sm leading-relaxed text-slate-400 font-medium">
                    ...
                    {/* IA Advisor Card */}
                    <div className="bg-gradient-to-br from-[#1e293b] to-[#0f172a] border border-[#f90f54]/30 p-7 rounded-[24px] shadow-2xl">
                      ...
                      <Button 
                        ...
                        className="w-full bg-[#f90f54]/10 hover:bg-[#f90f54]/20 text-[#f90f54] border border-[#f90f54]/30 font-black py-6 rounded-xl transition-all"
                      >
